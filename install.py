...
def configure_serial_port():
    ...
    if shutil.which('raspi-config') is not None:
        ...
        run('sudo bash', '''
if ! grep -q ^dtoverlay=miniuart-bt /boot/firmware/config.txt;
then
...
''')  # Switch the Bluetooth peripheral over to the Mini-UART, so we can use the PL011 UART.
        run('sudo raspi-config nonint set_config_var core_freq 250 /boot/firmware/config.txt')  # Fix the Mini-UART frequency.
